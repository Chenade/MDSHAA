...
<div class="content">
    ...
    $sql = 'SELECT * FROM `'.$_GET['year'].'`';
    $run = mysqli_query($con,$sql);

    while( $row = mysqli_fetch_assoc($run) ){
        echo '<button style="width:150px; height: 70px; margin: 40px; background-color: transparent; border-radius: 25px;" onclick=location.href="graduation_single.php?year='.$row['year'].'&class='.$row['class'].'">'.$row['class'].'</button>';
    }
    ?>
</div>
